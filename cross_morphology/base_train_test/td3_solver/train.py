...
def main(args):
    ...
    cprint('untrained policy evaluation return: {}'.format(evaluations))
    ...
    for t in range(int(args.max_timesteps)):
        ...
        if (t + 1) % args.eval_freq == 0:
            eval_rew = eval_policy(policy, args.env, args.seed)
            print(
                f"Total T: {t + 1} Episode Num: {episode_num + 1} Reward:{eval_rew}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_timesteps", default=int(4e5), type=int)  # Max time steps to run environment
