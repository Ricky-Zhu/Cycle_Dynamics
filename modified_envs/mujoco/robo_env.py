...
class Reach(SingleArmEnv):
    ...
    def __init__(
        ...
        action_low = self.action_spec[0]
        action_high = self.action_spec[1]
        self.action_space = Box(low=action_low, high=action_high)

        # define the observation space
        obs = np.concatenate(
            [self.observation_spec()["robot0_proprio-state"], self.observation_spec()["object-state"]])
        n = obs.shape[0]
        obs_low = np.full((n), -np.inf)
        obs_high = np.full((n), np.inf)
        self.observation_space = Box(low=obs_low, high=obs_high)

        self.reward_range = self.reward_scale
        self.metadata = None
        self.unwrapped = self
    ...
    def _post_action(self, action):
        ...
        gripper_site_pos = self.sim.data.site_xpos[self.robots[0].eef_site_id]
        dist = np.linalg.norm(gripper_site_pos - cube_pos)
        return reward, self.done or self._check_success(), {"success": self._check_success(), "distance": dist}
...
class ReachWrapper(Wrapper):
    ...
    def get_observation_shape(self):
        obs = np.concatenate(
            [self.env.observation_spec()["robot0_proprio-state"], self.env.observation_spec()["object-state"]])
        return obs.shape[0]
    ...
    def _process_obs(self, obs):
        obs_ = np.concatenate([obs["robot0_proprio-state"], obs["object-state"]])
        return obs_
